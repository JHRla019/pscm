	...
	<div class="search" style="padding : 1em 0 1em 0;">
		<form>
			<div class="search-part">
				<input id="searchingDate" name="timeSearch" type="text">
			...
			<div class="search-part">
				<span>프로그램ID</span> 
				<input type="text" name="text-programId">
			</div>
			<div class="search-part">
				<span>메인룰</span>
				<input type="text" name="text-mainRlue">
			</div>
		...
		<table>
			<tr>
				...
				<c:forEach var="i" begin="1" end="18">
					<th class="data">
						<c:choose>
							<c:when test="${i lt 10}">0<c:out value="${i}"/>시</c:when>
			    			<c:otherwise><c:out value="${i}"/>시</c:otherwise>
		    			</c:choose>
	    			</th>
				</c:forEach>
			</tr>
...
window.onload = function() {
	const calendar = flatpickr("#searchingDate", {dateFormat : "Y-m-d"});
	document.getElementById("searchingDate").value = calendar.formatDate(new Date(), "Y-m-d");
	navHighlight(document.getElementsByTagName('a')[1]);
}
</script>
